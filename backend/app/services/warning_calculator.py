...
def build_warnings(student_id, term, session=None):
    """
    生成某学生某学期的预警列表。
...
def get_warning_list(filters=None, page=1, page_size=20):
    """
    教师/管理员查询预警名单。
    ...
    for sid in student_ids:
        if not term:
            ...
            for t in terms:
                warns = build_warnings(sid, t[0])
                for w in warns:
                    w['student_id'] = sid
                all_warnings.extend(warns)
        else:
            warns = build_warnings(sid, term)
            for w in warns:
                w['student_id'] = sid
            all_warnings.extend(warns)
...
def refresh_warnings(term=None, class_name=None, operator_id=None, trace_id=None):
